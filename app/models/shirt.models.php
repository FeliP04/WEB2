...
class shirtModel
{
    ...
    function getShirts()
    {
        $query = $this->db->prepare('SELECT * FROM camiseta');
        $query->execute();
    ...
    //Modificar camiseta
    function updateShirt($id, $image, $id_team, $season, $type, $price)
    {
        $query = $this->db->prepare('UPDATE camiseta SET imagen = ?, id_equipo = ?, temporada = ?, tipo = ?, precio = ? WHERE id_camiseta = ?');
        $query->execute([$image, $id_team, $season, $type, $price, $id]);
    }
}
